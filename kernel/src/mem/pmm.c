...
void pmm_init(void) {
  assert(multiboot2_mmap);
  int num_entries
    = (multiboot2_mmap->size - (4 * sizeof(uint32_t))) / sizeof(multiboot2_tag_mem_map_entry_t);
  assert(num_entries > 0);

  memset(page_bitmap, 0xFFFFFFFF, sizeof(page_bitmap));

  for (int i = 0; i < num_entries; i++) {
    multiboot2_tag_mem_map_entry_t *entry = &multiboot2_mmap->mem_map.entries[i];
